...
gEcnu.Edit.graphMouseDownEvt=function(e,map){
	...
	switch (mode) {
	    ...
	    case 'drawLine':
	       ...
	       if(len>1){
	       	  //TODO_此时需要绘制多段线
	       	   var line_line_style=new gEcnu.Style({opacity:1,fillColor:'#118811',strokeColor:'#118811',lineWeight:1})
...
gEcnu.Edit.graphMouseMoveEvt = function (e, map) {
	...
	switch (mode) {
	    case 'drawLine':
	        ...
	        if (len >= 1) {
			    //map.overLayer.clear();
			    var line_point_style=new gEcnu.Style({opacity:1,fillColor:'#118811',strokeColor:'#118811',lineWeight:1});
	            map.overLayer.setStyle(line_point_style);
	            var ctx=map.overLayer._ctx;
	            gEcnu.Util.setStyle(ctx,line_point_style);
