	...
	*/
{
	...
	_initEvent: function () {
        ...
        function mapMouseDownEvt_pan(e){
            ...
            if(gSelf.mode == 'map'&&(gSelf.mapTool != "rulerLength")&&(gSelf.mapTool != "rulerArea")){
                //地图浏览
	...
	zoomIn: function (mouseXY) {
		...
		if(this.ownOther){  
			...
			if(curoMaplevel>=gSelf.activeLayer.oMap.MaxLevel){  alert("已经放大至最大级别");
					return;
				}
		  }
	...
	zoomOut: function (mouseXY) {
		...
		if(this.ownOther){
			...
			if(curoMaplevel<=gSelf.activeLayer.oMap.Minlevel+1){  alert("已经缩小至最小级别");
					return;
				}
		  }	
		this.zoom = (this.zoom) * 2;
