...
gEcnu.WebFeatureServices.QueryByGeometry = gEcnu.WebFeatureServices.extend({
    ...
    hightLightFeatures:function (resultFeatures){ 
        //console.log('hightlight',resultFeatures);
        var map=gSelf;
        map.overLayer.clear();
        var fea_len=resultFeatures.length;  
        for(var i=0;i<fea_len;i++){  
            var tmpfea=resultFeatures[i];
            var shptype=tmpfea.shape.shpType;
            if(shptype==5){   //高亮显示在覆盖层上 
                var len=tmpfea._lineRings.length;
                for(var j=0;j<len;j++){
                    var points=tmpfea._lineRings[j].points;
                    //进行边界高亮的重绘操作
                    var line_style=new gEcnu.Style({ opacity:1,fillColor:'',strokeColor:'#00FFFF',lineWeight:2});
                ...
                var len=tmpfea._lineStrings.length;
                for(var j=0;j<len;j++){
                    var points=tmpfea._lineStrings[j].points;
                    //进行边界的重绘操作               
                    var line_style=new gEcnu.Style({ opacity:1,fillColor:'',strokeColor:'#00FFFF',lineWeight:2});
                    map.overLayer.setStyle(line_style);
                    var ctx=map.overLayer._ctx;
                ...
                var points=tmpfea._gpoints;
                var point_style=new gEcnu.Style({cirRadius:4,opacity:1,fillColor:'#00FFFF'});
                var ctx=map.overLayer._ctx;
