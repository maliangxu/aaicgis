...
gEcnu.Layer.Dyn = gEcnu.Layer.extend({
	...
	_initParams: function (map) {
		...
		this.fileServer = map.dynFileServer;
		this._params = {
			map: this.ws,
			w: size.w,
			h: size.h,
...
gEcnu.Layer.Tile_Ex = gEcnu.Layer.extend({
	...
	_initProp: function (map) {
		...
		this.nWidth = Math.ceil(parseInt(size.w) / map.tileWidth) + 1;
		this.nHeight = Math.ceil(parseInt(size.h) / map.tileHeight) + 1;
		this.tileMapURL = "https://ccgis.cn/mapb/webtilemap";//map.tileMapURL_Ex; 
		this.fileServer = "https://ccgis.cn/mapb/fileServer?fn=map/"; //map.fileServer;
		this.mapTool = 'pan';
		this.ifInit = false;
		this.ifZoomTo = false;
		this.requestTimer=null;
